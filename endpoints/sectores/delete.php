...
if (!empty($id)) {
    // Se prepara la consulta SQL para eliminar el sector.
    $query = "UPDATE sector SET Visible = 0 WHERE ID = :id";
    ...
    if ($stmt->execute()) {
        ...
        // Se envía una respuesta JSON indicando que el sector fue eliminado.
        echo json_encode(array("message" => "El sector " . $id . " fue eliminado."));
    } else {
        // Si la ejecución falla, se establece el código de respuesta a 503 Service Unavailable.
        http_response_code(503);
        // Se envía una respuesta JSON indicando que no se pudo eliminar el sector.
        echo json_encode(array("message" => "No fue posible eliminar el sector."));
    }
} else {
    ...
    echo json_encode(array("message" => "Falta el id del sector."));
}