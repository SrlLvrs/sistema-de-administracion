...
if (!empty($id)) {
    ...
    $query = "UPDATE productos SET Visible = 0 WHERE ID = :id";
    ...
    if ($stmt->execute()) {
        ...
        // Se envía una respuesta JSON indicando que el PRODUCTO fue eliminado.
        echo json_encode(array("message" => "El PRODUCTO " . $id . " fue eliminado."));
    } else {
        // Si la ejecución falla, se establece el código de respuesta a 503 Service Unavailable.
        http_response_code(503);
        // Se envía una respuesta JSON indicando que no se pudo eliminar el producto.
        echo json_encode(array("message" => "No se ha podido eliminar el producto. Revisar error_log."));
    }
} else {
