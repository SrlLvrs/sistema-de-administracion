...
*/
$idpedido = isset($_GET['idpedido']) ? $_GET['idpedido'] : ''; //NOT NULL
...
if (!empty($idpedido)) {
    ...
    $query = "  UPDATE pedidos_productos 
                SET Cantidad = :c, Total = :t
                WHERE IDPedido = :idp AND IDProducto = :idpro
                LIMIT 1";
    ...
    $stmt->bindParam(":idpro", $idproducto);
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(array("message" => "El producto " . $idproducto. " con cantidad " . $cantidad . " en el pedido " . $idpedido . " fue actualizado."));
    } else {
